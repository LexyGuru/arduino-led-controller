...
// ========================= ARDUINO CONSOLE CACHE =========================

// A firmware memoriakimeles miatt egy valaszban csak nehany naplosort ad.
// A szerver ezert lapozva, az `after` azonosito hasznalataval gyujti ossze a
// teljes gyurupuffert, majd csak az uj bejegyzeseket keri le.
const ARDUINO_CONSOLE_CACHE_LIMIT = 200;
const ARDUINO_CONSOLE_MAX_PAGES = 24;
const arduinoConsoleCache = { logs: [], lastId: 0, updatedAt: 0, loading: null };
...
function consoleValueToText(value) {
  ...
  if (typeof value === 'object') {
    ...
    try { return JSON.stringify(value); } catch (_) { return '[Nem megjelenitheto objektum]'; }
  }
  return String(value);
}

function normalizeArduinoConsoleLog(raw, fallbackId = 0) {
  const source = raw && typeof raw === 'object' ? raw : { message: raw };
  const parsedId = Number(source.id);
  const id = Number.isFinite(parsedId) && parsedId >= 0 ? parsedId : fallbackId;
  let type = consoleValueToText(source.type || source.level || source.severity || 'info').toLowerCase();
  if (!['console', 'info', 'success', 'warn', 'warning', 'error', 'output'].includes(type)) type = 'info';
  if (type === 'warning') type = 'warn';

  let messageValue = source.message;
  if (messageValue == null) messageValue = source.text;
  if (messageValue == null) messageValue = source.msg;
  if (messageValue == null) messageValue = source.event;
  if (messageValue == null && raw && typeof raw === 'object') {
    const details = { ...raw };
    delete details.id; delete details.timestamp; delete details.type; delete details.level; delete details.severity;
    messageValue = Object.keys(details).length ? details : '';
  }

  const timestampValue = source.timestamp ?? source.time ?? source.uptime ?? '';
  return {
    id,
    timestamp: consoleValueToText(timestampValue),
    type,
    message: consoleValueToText(messageValue)
  };
}

function resetArduinoConsoleCache() {
  arduinoConsoleCache.logs = [];
  arduinoConsoleCache.lastId = 0;
  arduinoConsoleCache.updatedAt = 0;
}

async function refreshArduinoConsoleCache(force = false) {
  if (arduinoConsoleCache.loading) return arduinoConsoleCache.loading;
  if (!force && arduinoConsoleCache.updatedAt && Date.now() - arduinoConsoleCache.updatedAt < 2500) {
    return { lastId: arduinoConsoleCache.lastId, logs: arduinoConsoleCache.logs, cachedAt: arduinoConsoleCache.updatedAt };
  }

  arduinoConsoleCache.loading = (async () => {
    let after = arduinoConsoleCache.lastId;
    let cacheResetDetected = false;

    for (let page = 0; page < ARDUINO_CONSOLE_MAX_PAGES; page++) {
      const payload = await arduino.get(`/api/console/logs?after=${Math.max(0, after)}`);
      const pageLogs = Array.isArray(payload) ? payload : (Array.isArray(payload?.logs) ? payload.logs : []);
      const responseLastId = Number(Array.isArray(payload) ? 0 : payload?.lastId) || 0;

      // Firmware-ujrainditas utan a naploazonositok ujra 1-tol indulhatnak.
      // Ilyenkor a regi cache-t el kell dobni, kulonben regi sorok maradnanak.
      if (after > 0 && responseLastId > 0 && responseLastId < after && !cacheResetDetected) {
        resetArduinoConsoleCache();
        after = 0;
        cacheResetDetected = true;
        page--;
        continue;
      }

      if (!pageLogs.length) break;
      const normalized = pageLogs.map((entry, index) => normalizeArduinoConsoleLog(entry, after + index + 1));
      const merged = new Map(arduinoConsoleCache.logs.map((entry) => [entry.id, entry]));
      for (const entry of normalized) merged.set(entry.id, entry);
      arduinoConsoleCache.logs = [...merged.values()]
        .sort((left, right) => left.id - right.id)
        .slice(-ARDUINO_CONSOLE_CACHE_LIMIT);

      const highestPageId = normalized.reduce((highest, entry) => Math.max(highest, Number(entry.id) || 0), 0);
      const nextAfter = Math.max(responseLastId, highestPageId);
      if (nextAfter <= after) break;
      after = nextAfter;
      arduinoConsoleCache.lastId = nextAfter;
    }

    arduinoConsoleCache.updatedAt = Date.now();
    return {
      lastId: arduinoConsoleCache.lastId,
      logs: arduinoConsoleCache.logs,
      cachedAt: arduinoConsoleCache.updatedAt
    };
  })().finally(() => { arduinoConsoleCache.loading = null; });

  return arduinoConsoleCache.loading;
}
...
app.get('/api/arduino/console/logs', async (req, res) => {
  try {
    const force = req.query.refresh === '1' || req.query.force === '1';
    res.json(await refreshArduinoConsoleCache(force));
  } catch (error) {
    logger.error(`Arduino console log error: ${error.message}`);
    res.status(502).json({ error: error.message, code: 'ARDUINO_CONSOLE_ERROR', timestamp: new Date().toISOString() });
  }
});
...
app.post('/api/arduino/console/clear', async (req, res) => {
  try {
    const result = await arduino.get('/api/console/clear');
    resetArduinoConsoleCache();
    if (io) io.emit('consoleCleared', result);
    res.json(result);
  } catch (error) {
    logger.error(`Arduino console clear error: ${error.message}`);
    res.status(502).json({ error: error.message, code: 'ARDUINO_CONSOLE_ERROR', timestamp: new Date().toISOString() });
  }
});
...
async function request(url,options={}){const response=await fetch(url,{cache:'no-store',headers:{'Content-Type':'application/json'},...options});const data=await response.json().catch(()=>({}));if(!response.ok)throw new Error(data.error||'Ismeretlen szerverhiba');return data}
...
function logSeconds(value){if(typeof value==='number'&&Number.isFinite(value))return Math.max(0,value);const match=String(value||'').match(/(\d+(?:\.\d+)?)\s*s/i);return match?Math.max(0,Number(match[1])):null}
function logClock(value,uptime){const seconds=logSeconds(value),current=Number(uptime);if(seconds!=null&&Number.isFinite(current)&&current>=seconds){const when=new Date(Date.now()-(current-seconds)*1000);return when.toLocaleTimeString('hu-HU',{hour:'2-digit',minute:'2-digit',second:'2-digit'})}return value||'—'}
function logText(value){if(value==null)return '';if(typeof value==='string')return value;if(typeof value==='number'||typeof value==='boolean')return String(value);if(typeof value==='object'){if(typeof value.message==='string')return value.message;try{return JSON.stringify(value)}catch(_){return '[Nem megjeleníthető objektum]'}}return String(value)}
async function loadLogs(force){if(logsLoading)return;logsLoading=true;const button=document.getElementById('refreshLogs'),original=button?button.textContent:'';if(button){button.disabled=true;button.textContent='Frissítés…'}try{const suffix=force?'?refresh=1&_='+Date.now():'?_='+Date.now(),results=await Promise.all([request('/api/arduino/console/logs'+suffix),request('/api/arduino/console/stats?_='+Date.now())]);const logs=Array.isArray(results[0])?results[0]:(results[0].logs||[]),stats=results[1]||{};document.getElementById('logCount').textContent=logs.length;document.getElementById('consoleUptime').textContent=formatUptime(stats.uptime);document.getElementById('consoleSignal').textContent=stats.wifiSignal==null?'—':stats.wifiSignal+' dBm';document.getElementById('consoleState').textContent=stats.system&&stats.system.consoleActive?'Aktív':'—';document.getElementById('logsUpdatedAt').textContent='Utolsó frissítés: '+new Date().toLocaleTimeString('hu-HU',{hour:'2-digit',minute:'2-digit',second:'2-digit'});const root=document.getElementById('consoleLogs');root.replaceChildren();if(!logs.length){const empty=document.createElement('div');empty.className='muted';empty.textContent='Nincs naplóbejegyzés.';root.append(empty);return}logs.slice(-100).reverse().forEach(function(log){const row=document.createElement('div'),time=document.createElement('time'),type=document.createElement('span'),message=document.createElement('span'),kind=typeof log.type==='string'?log.type.toLowerCase():'info';row.className='log '+kind;time.textContent=logClock(log.timestamp,stats.uptime);time.title='Arduino üzemidő: '+(log.timestamp||'—');type.className='log-type';type.textContent=kind.toUpperCase();message.className='log-message';message.textContent=logText(log.message!=null?log.message:log);row.append(time,type,message);root.append(row)})}catch(error){const root=document.getElementById('consoleLogs');root.replaceChildren();const failure=document.createElement('div');failure.className='muted';failure.textContent='A napló jelenleg nem érhető el: '+error.message;root.append(failure)}finally{logsLoading=false;if(button){button.disabled=false;button.textContent=original||'Napló frissítése'}}}
