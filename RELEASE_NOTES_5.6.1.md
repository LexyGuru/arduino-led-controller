...
- The running `5.6.1` build always identifies itself as a Beta build.
...
1. Launch the released `5.6.1` application.
...
5. Switch Firmware Update Channel back to Beta and verify `5.0.0` is available.
...
## Stable Device Key transport recovery

- Fixed a cross-layer Device Key contract mismatch.
- Native credential storage already accepted valid 16-byte Device Keys, while the HTTP `X-Device-Key` transport incorrectly required at least 24 characters.
- The HTTP transport now accepts the same 16-byte minimum as credential storage.
- Printable ASCII `0x21..0x7e` validation remains enforced, so control characters, whitespace and non-header-safe values are still rejected.
- Firmware remains `5.0.0`; Direct API remains `1.0.0`.

## Stable promotion

This Stable release is promoted from the manually validated `5.6.1-beta.2` application and `5.0.0-beta.10` firmware candidate.
The Device Key cross-layer contract fix is included: valid printable-ASCII keys use the same 16-character minimum in secure storage and HTTP transport.
