...
export interface FirmwareStatus { state: string; message: string; installedVersion?: string; arduinoOnline: boolean; otaToolInstalled: boolean; otaPasswordConfigured: boolean; availableFirmware?: FirmwareArtifact; firmwareLookupError?: string; otaToolPath?: string; otaToolError?: string; otaTargetAddress?: string; otaTargetPort?: number; updateAvailable: boolean; progress?: number; phase?: string; }
export interface OtaProgressEvent { timestamp: number; stage: string; level: 'info' | 'success' | 'error' | 'output'; message: string; progress?: number; }
