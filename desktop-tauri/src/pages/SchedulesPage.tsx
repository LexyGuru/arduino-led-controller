...
import { AlertTriangle, CheckCircle2, CopyPlus, Database, Download, Pencil, RefreshCw, Save, Trash2, Upload } from 'lucide-react';
import { open, save } from '@tauri-apps/plugin-dialog';
import { useV5Schedules } from '../hooks/useV5Schedules';
import { tauriApi } from '../services/tauriApi';
import { scheduleFingerprint } from '../services/v5ScheduleModels.mjs';
import { useI18n } from '../i18n';
import type { LedSchedule, ScheduleBackup, ScheduleLed, ScheduleSaveProgressEvent, ScheduleSaveResult, ScheduleSyncSnapshot, ScheduleSyncState } from '../types';
const tauriRuntime = typeof globalThis !== 'undefined' && '__TAURI_INTERNALS__' in globalThis;
async function pickBrowserJsonFile(): Promise<LedSchedule[] | null> { return new Promise((resolve, reject) => { const input = document.createElement('input'); input.type = 'file'; input.accept = 'application/json,.json'; input.onchange = async () => { try {
    ...
    const schedules = Array.isArray(value) ? value : Array.isArray(value?.schedules) ? value.schedules : null;
    if (!schedules)
        throw new Error('A JSON nem tartalmaz schedules listát.');
    resolve(schedules as LedSchedule[]);
}
catch (error) {
    reject(error);
} }; input.click(); }); }
...
const emptyLeds = (): FormLed[] => [1, 2, 3].map((id) => ({
    id,
    action: 'none',
    brightness: 10,
    effect: 0,
...
export function SchedulesPage({ schedules: legacySchedules, scheduleSync, busy: legacyBusy, onSave: directSave, onSync: directSync }: {
    schedules: LedSchedule[];
    scheduleSync: ScheduleSyncState;
    busy: boolean;
    onSave: (schedules: LedSchedule[], expectedRevision: number | null, force?: boolean) => Promise<ScheduleSaveResult>;
    onSync: () => Promise<ScheduleSyncSnapshot>;
}) {
    const { t, language } = useI18n();
    const state = useV5Schedules({
        legacySchedules,
        legacyBusy,
    ...
    const [draft, setDraft] = useState<LedSchedule[]>(state.schedules);
    const [baseFingerprint, setBaseFingerprint] = useState(state.remoteFingerprint);
    const [baseRevision, setBaseRevision] = useState<number | null>(state.remoteRevision);
    ...
    const [saveProgress, setSaveProgress] = useState<ScheduleSaveProgressEvent | null>(null);
    ...
    const [copyTargetDays, setCopyTargetDays] = useState<number[]>([2]);
    const dirty = scheduleFingerprint(draft) !==
        baseFingerprint;
    useEffect(() => {
        if (!dirty) {
            setDraft(state.schedules);
            setBaseFingerprint(state.remoteFingerprint);
            setBaseRevision(state.remoteRevision);
            setConflict(false);
        }
        else if (state.remoteFingerprint !==
            baseFingerprint ||
    ...
    const deleteAllSchedules = async () => {
        if (!state.canWrite || draft.length === 0)
            return;
        const confirmation = globalThis.prompt(t('schedules.deletePrompt', { count: draft.length }));
        if (confirmation !== t('schedules.deleteToken'))
            return;
        try {
            const backup = await tauriApi.createScheduleBackup(draft, baseRevision, state.syncState.checksum);
    ...
    const restoreScheduleBackup = async (backup: ScheduleBackup) => {
        if (!state.canWrite)
            return;
        if (!globalThis.confirm(t('schedules.restoreConfirm', { count: backup.count })))
            return;
        try {
    ...
    const grouped = useMemo(() => dayKeys.map((key, index) => ({
        name: t(key),
        day: index + 1,
        items: draft
            .filter((item) => item.day ===
            index + 1)
            .sort((left, right) => left.time
            .localeCompare(right.time))
    })), [draft, t]);
    const totalScheduleActions = grouped.reduce((total, day) => total + day.items.length, 0);
    const scheduleDaysUsed = grouped.filter((day) => day.items.length > 0).length;
    const copyDaySchedules = () => {
        const source = draft.filter((item) => item.day ===
            copySourceDay);
        if (!source.length ||
            !copyTargetDays.length) {
            setFileMessage(t('beta2.schedule.empty'));
            return;
        }
        const targetSet = new Set(copyTargetDays.filter((day) => day !==
            copySourceDay));
        if (!targetSet.size) {
            setFileMessage(t('beta2.schedule.sameDayOnly'));
            return;
        }
        const sourceTimes = new Set(source.map((item) => item.time));
        setDraft((current) => {
            const remaining = current.filter((item) => !(targetSet.has(item.day) &&
                sourceTimes.has(item.time)));
            const copied = [...targetSet]
                .flatMap((day) => source.map((item) => ({
                ...item,
                id: crypto.randomUUID(),
                day,
                leds: item.leds.map((led) => ({
                    ...led,
                    color: [...led.color] as [
                        number,
                        number,
            ...
            return [
                ...remaining,
                ...copied
            ].sort((left, right) => left.day -
                right.day ||
                left.time.localeCompare(right.time));
        });
        setFileMessage(`${t('beta2.schedule.copied', { source: source.length, targets: targetSet.size })} ${t('beta2.schedule.saveHint')}`);
    };
    const setFormLed = (id: number, patch: Partial<FormLed>) => setFormLeds((items) => items.map((item) => item.id === id
        ? {
            ...item,
            ...patch
        }
    ...
    const saveDraft = async (force = false) => {
        try {
            ...
            const result = await state.save(draft, {
    ...
    const addOrUpdate = () => {
        const leds: ScheduleLed[] = formLeds
            .filter((led) => led.action !==
            'none')
            .map((led) => ({
            id: led.id,
            enabled: led.action ===
        ...
        setDraft((items) => {
            const remaining = editingId
                ? items.filter((item) => item.id !==
                    editingId)
                : items;
            const additions = selectedDays.map((day) => ({
                id: crypto
                    .randomUUID(),
                day,
                time,
    ...
    const edit = (schedule: LedSchedule) => {
        ...
        setFormLeds([1, 2, 3].map((id) => {
            const led = schedule.leds
                .find((entry) => entry.id ===
                id);
            return led
                ? {
    ...
    const editAndReveal = (schedule: LedSchedule) => {
        edit(schedule);
    ...
    const exportJson = async () => {
        try {
            ...
            const file = new File([json], 'weekly-led-schedules.json', { type: 'application/json' });
            if (typeof navigator.share === 'function' &&
                ...
                setFileMessage(t('schedules.exportShared', { count: draft.length, bytes: json.length }));
                return;
            }
            if (!tauriRuntime) {
            ...
            await tauriApi
                .exportSchedulesFile(path, draft);
            setFileMessage(t('schedules.exportSaved', { count: draft.length, path, bytes: json.length }));
        }
        catch (error) {
            setFileMessage(t('schedules.exportError', { error: String(error) }));
        }
    };
    const importJson = async () => {
        try {
            ...
            const imported = await tauriApi
                .importSchedulesFile(selected);
            setDraft(imported);
            setFileMessage(t('schedules.imported', { count: imported.length }));
        }
        catch (error) {
            setFileMessage(t('schedules.importError', { error: String(error) }));
        }
    };
    return (<div className="page visual31-management-page visual31-schedules-page v55-schedules-page beta4-schedules-redesign core-v3-management-page core-v3-schedules-page" data-core-management="schedules">
      ...
      <section className="schedule-mobile-workspace" data-schedule-mobile="v1">
        <div className="schedule-mobile-week" role="tablist" aria-label={t('visual31.schedule.timelineTitle')}>
          {grouped.map((day, index) => (<button type="button" role="tab" key={`mobile-${day.day}`} className={`schedule-mobile-day ${activeScheduleDay === day.day ? 'is-active' : ''} ${day.items.length ? 'has-events' : ''}`} aria-selected={activeScheduleDay === day.day} onClick={() => {
        ...
        <div className="schedule-mobile-daybar">
          ...
          <button type="button" className="schedule-mobile-add" disabled={!state.canWrite} onClick={() => {
            resetForm();
        ...
        <div className="schedule-mobile-events">
          {(grouped.find((day) => day.day === activeScheduleDay)?.items ?? []).length === 0 ? (<div className="schedule-mobile-empty">{t('schedules.empty')}</div>) : ((grouped.find((day) => day.day === activeScheduleDay)?.items ?? []).map((schedule) => (<article className="schedule-mobile-event" key={`mobile-event-${schedule.id}`}>
                <button type="button" className="schedule-mobile-event-main" disabled={!state.canWrite} onClick={() => {
                edit(schedule);
                setScheduleModalOpen(true);
            }}>
                  <strong>{schedule.time}</strong>
                  <div>
                    {schedule.leds.map((led, index) => (<span key={`${schedule.id}-m-${index}`}>
                        LED {index + 1}
                        {' · '}
                        {led.enabled ? 'ON' : 'OFF'}
                        {' · '}
                ...
                setScheduleModalOpen(true);
                  ...
                  <button type="button" className="danger" aria-label={t('common.delete')} disabled={!state.canWrite} onClick={() => setDraft((items) => items.filter((item) => item.id !== schedule.id))}>
                    <Trash2 size={18}/>
                  </button>
                </div>
              </article>)))}
      ...
      <section className="panel visual31-management-schedule-timeline schedule-unified-workspace schedule-desktop-workspace" data-schedule-unified="v1">
        ...
        <div className="visual31-week-grid">
          {grouped.map((day) => (<button type="button" key={day.day} className={`visual31-week-day schedule-unified-day-tab ${day.items.length ? 'has-events' : 'is-empty'} ${activeScheduleDay === day.day ? 'is-selected' : ''}`} aria-pressed={activeScheduleDay === day.day} onClick={() => {
                setActiveScheduleDay(day.day);
                setSelectedDays([day.day]);
                setEditingId(null);
                setScheduleModalOpen(true);
        ...
        {scheduleModalOpen && createPortal((<div className="schedule-modal-backdrop" role="presentation" onMouseDown={(event) => {
                if (event.currentTarget === event.target) {
                    setScheduleModalOpen(false);
                    setEditingId(null);
                }
            }}>
            <section className="schedule-modal-panel" role="dialog" aria-modal="true" aria-labelledby="schedule-modal-title" onMouseDown={(event) => event.stopPropagation()}>
              <header className="schedule-modal-header">
                <div>
                  <p className="eyebrow"><I18nText k="legacyUi.heti.idozites.add10fe6"/></p>
                  <h2 id="schedule-modal-title">
                    {grouped.find((day) => day.day === activeScheduleDay)?.name ?? '—'}
                  </h2>
                </div>
                <button type="button" className="schedule-modal-close" aria-label={t("core.close")} onClick={() => {
                setScheduleModalOpen(false);
                setEditingId(null);
            }}>
                  ×
        ...
        <div className="schedule-modal-day-list">
          {grouped.filter((group) => group.day === activeScheduleDay).map((group) => (<article className="schedule-day-card" key={group.day}>
              ...
              <div className="schedule-day-items">
                {group.items.length === 0 ? (<div className="schedule-day-empty">
                    {t('schedules.empty')}
                  </div>) : (group.items.map((schedule) => (<div className="schedule-compact-card" key={schedule.id}>
                      <button type="button" className="schedule-compact-time" disabled={!state.canWrite} onClick={() => edit(schedule)}>
                        {schedule.time}
                      </button>

                      <div className="schedule-led-chips">
                        {schedule.leds.length ? (schedule.leds.map((led) => (<span className={`schedule-led-chip ${led.enabled ? 'is-on' : 'is-off'}`} key={led.id} title={t('schedules.ledSummary', {
                            id: led.id,
                            state: t(led.enabled ? 'common.on' : 'common.off'),
                            brightness: led.brightness,
                            color: led.color.join(',')
                        })}>
                              <i className="schedule-color-dot" style={{ backgroundColor: `rgb(${led.color.join(',')})` }}/>
                              LED {led.id} · {t(led.enabled ? 'common.on' : 'common.off')} · {led.brightness}
                            </span>))) : (<span className="schedule-no-action">
                            {t('schedules.noLedAction')}
                          </span>)}
                      </div>

                      <div className="schedule-compact-actions schedule-event-actions">
                        <button type="button" className="icon-button secondary schedule-compact-edit schedule-event-action schedule-event-action-edit" title={t('schedules.edit')} aria-label={t('schedules.edit')} disabled={!state.canWrite} onClick={() => editAndReveal(schedule)}>
                          <Pencil size={15}/>
                        </button>

                      <button type="button" className="icon-button danger schedule-compact-delete schedule-event-action schedule-event-action-delete" title={t('common.delete')} disabled={!state.canWrite} onClick={() => setDraft((items) => items.filter((item) => item.id !== schedule.id))}>
                        <Trash2 size={15}/>
                      </button>
                      </div>
                    </div>)))}
        ...
        <div className="schedule-unified-editor-head">
          <div>
            <p className="eyebrow"><I18nText k="legacyUi.aktiv.nap.f44be01b"/></p>
            <h3>{grouped.find((day) => day.day === activeScheduleDay)?.name ?? '—'}</h3>
          </div>
          <span>{grouped.find((day) => day.day === activeScheduleDay)?.items.length ?? 0}<I18nText k="legacyUi.esemeny.de4d41cb"/></span>
        </div>

        <div className="schedule-top-row">
          <label>
            {t('schedules.time')}
            <input type="time" value={time} onChange={(event) => setTime(event.target
                .value)}/>
          </label>

          <div className="day-picker">
            <label className="select-all">
              <input type="checkbox" checked={selectedDays
                .length ===
                7} onChange={(event) => setSelectedDays(event.target
                .checked
                ? [
                    1,
                    2,
            ...
            <div className="day-buttons">
              {dayShortKeys.map((key, index) => (<button type="button" key={key} className={selectedDays
                    .includes(index + 1)
                    ? 'day-active'
                    : 'secondary'} onClick={() => setSelectedDays((current) => current
                    .includes(index + 1)
                    ? current
                        .filter((day) => day !==
                        index +
        ...
        <div className="schedule-led-grid">
          {formLeds.map((led) => (<article className="schedule-led-card" key={led.id}>
                <h3>
                  LED {led.id}
                </h3>
                ...
                <label>
                  ...
                  <select value={led.action} onChange={(event) => setFormLed(led.id, {
                    action: event
                        .target
                        .value as Action
                })}>
                ...
                <label>
                  ...
                  <div className="color-row">
                    <input type="color" disabled={led.action ===
                    'none'} value={rgbToHex(led.color)} onChange={(event) => setFormLed(led.id, {
                    color: hexToRgb(event
                        .target
                        .value)
                })}/>
                ...
                <label>
                  ...
                  <input type="range" min="0" max="255" disabled={led.action ===
                    'none'} value={led.brightness} onChange={(event) => setFormLed(led.id, {
                    brightness: Number(event
                        .target
                        .value)
                })}/>
                </label>
                ...
                <label>
                  ...
                  <select disabled={led.action ===
                    'none'} value={led.effect} onChange={(event) => setFormLed(led.id, {
                    effect: Number(event
                        .target
                        .value)
                })}>
                    {effectKeys.map((key, index) => (<option value={index} key={key}>
                          {t(key)}
                        </option>))}
                  </select>
                </label>
        ...
        <div className="v5-actions">
          <button className="schedule-add" disabled={state.busy ||
                !state.canWrite ||
                !selectedDays
                    .length ||
                formLeds.every((led) => led.action ===
                    'none')} onClick={addOrUpdate}>
            {editingId
                ? t('schedules.edit')
                : t('schedules.new')}
          </button>

          {editingId && (<button className="secondary" onClick={resetForm}>
      ...
      <div className="page-heading v55-management-heading core-v3-management-heading schedule-unified-actions">
        ...
        <div className="page-actions core-v3-management-actions">
          <button onClick={() => void saveDraft()} disabled={state.busy ||
            !dirty ||
            !state.canWrite}>
            <Save size={17}/>
          ...
          <button className="secondary" onClick={() => void reloadRemote()} disabled={state.busy}>
            <RefreshCw size={17}/>
            {t('schedules.loadArduino')}
          </button>

          <button className="secondary" onClick={() => void exportJson()} disabled={state.busy}>
            <Download size={17}/>
            {t('schedules.saveJson')}
          </button>

          <button className="secondary" onClick={() => void importJson()} disabled={state.busy}>
            <Upload size={17}/>
            {t('schedules.loadJson')}
          </button>

          <button className="danger" onClick={() => void deleteAllSchedules()} disabled={state.busy || !state.canWrite || draft.length === 0}>
            <Trash2 size={17}/>
      ...
      <section className="panel">
        <div className="panel-title">
          <div>
            ...
            <h2><I18nText k="legacyUi.schedule.szinkron.1808a18c"/></h2>
          </div>
        ...
        {!state.canWrite && (<div className="notice">
        ...
        {state.syncState.recoveredLegacyActionCount > 0 && (<div className="notice">
            <AlertTriangle size={18}/>
            <p>
        ...
        {state.syncState.emptyActionCount > 0 && (<div className="notice">
            <AlertTriangle size={18}/>
            <p>
      ...
      {saveProgress && (<section className="schedule-save-progress-card" data-level={saveProgress.level} aria-live="polite">
        ...
        <div className="beta2-copy-grid">
          <label>
            ...
            <select value={copySourceDay} onChange={(event) => setCopySourceDay(Number(event.target.value))}>
              {dayKeys.map((key, index) => (<option key={key} value={index + 1}>
                  {t(key)}
                </option>))}
            </select>
          </label>
          ...
          <div>
            ...
            return (<button key={key} type="button" className={active ? 'secondary active' : 'secondary'} disabled={day === copySourceDay} onClick={() => setCopyTargetDays((current) => current.includes(day)
          ...
          <button type="button" className="secondary" disabled={state.busy ||
            !draft.some((item) => item.day === copySourceDay)} onClick={copyDaySchedules}>
            <CopyPlus size={17}/>
      ...
      {conflict && (<section className="v5-schedule-conflict">
          ...
          <div className="v5-actions">
            <button className="secondary" disabled={state.busy} onClick={() => void reloadRemote()}>
              <RefreshCw size={16}/>
              {t('schedules.reload')}
            </button>
          </div>
      ...
      {(fileMessage ||
            state.notice ||
            state.error) && (<section className="panel">
          {fileMessage && (<p className="notice-text">
              {fileMessage}
            </p>)}

          {state.notice && (<p className="notice-text">
              {state.notice}
            </p>)}
      ...
      <section className="panel">
        ...
        {backups.length === 0 ? (<p className="muted">{t('schedules.noBackups')}</p>) : (<div className="v5-backup-list">
            {backups.slice(0, 10).map((backup) => (<article key={backup.id}>
                <div>
      ...
      {!draft.length && (<section className="panel empty">
          <h3>
            {t('schedules.empty')}
          </h3>
          <p>
