...
export function FirmwarePage({ firmware, busy, onRefresh, onUpdate }: { firmware: FirmwareStatus | null; busy: boolean; onRefresh: () => void; onUpdate: () => void }) {
  ...
  return <div className="page">
    ...
    <section className="stats-grid">
      ...
      <article className="stat-card"><small>OTA feltöltő</small><strong>{firmware?.otaToolInstalled ? 'Telepítve' : 'Hiányzik'}</strong></article>
    ...
    <section className="panel firmware-panel">
      ...
      <div><h3>{firmware?.message ?? 'Kattints az ellenőrzésre.'}</h3><p>{available ? `${available.name} • ${available.createdAt ?? available.tag}` : firmware?.firmwareLookupError ?? 'A GitHub firmware-kiadás még nincs lekérve.'}</p></div>
      <button onClick={onUpdate} disabled={busy || !firmware?.otaToolInstalled || !firmware?.otaPasswordConfigured || !available}><UploadCloud size={18}/> Firmware telepítése</button>
    </section>
    <div className="notice"><DownloadCloud size={18}/> A program a GitHub <b>firmware-latest</b> kiadásából tölti le a `.ino.bin` fájlt, ellenőrzi a SHA-256 értéket, majd a helyi `arduinoOTA` feltöltővel telepíti.</div>
  </div>;
}
