...
export function SettingsPage({platform,otaSupported,config,busy,onChange,onPersist,onSave,onTest,otaPassword,onOtaPasswordChange,appUpdate}:SettingsPageProps){
 ...
 const {t,language,setLanguage}=useI18n(); const [settingsView,setSettingsView]=useState<'general'|'arduino'>('general'); const set=<K extends keyof ConnectionConfig>(key:K,value:ConnectionConfig[K])=>{const next={...config,[key]:value};commit(next,persistentKeys.has(key));};
