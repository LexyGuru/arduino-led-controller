...
export function SettingsPage({ platform, otaSupported, config, busy, onChange, onPersist, onSave, onTest, otaPassword, onOtaPasswordChange, appUpdate, target = 'general', onTargetChange }: SettingsPageProps) {
    ...
    const [settingsView, setSettingsView] = useState<SettingsTarget>(target);
    const selectSettingsView = (next: SettingsTarget) => {
        setSettingsView(next);
        onTargetChange?.(next);
    };
    useEffect(() => {
        if (target !== settingsView) setSettingsView(target);
    }, [target]);
    ...
    useEffect(() => { if (settingsView === 'hardware' && !ledTopology && !ledTopologyBusy)
    ...
    return <div className="page visual31-management-page visual31-settings-page settings-page v55-settings-hub beta4-settings-redesign beta5-settings-redesign core-v3-management-page core-v3-settings-page" data-settings-view={settingsView} data-core-management="settings">
 ...
 <div className="visual31-management-kpis">
  ...
  <div className="v55-settings-hub-tabs core-v3-settings-tabs beta5-settings-tabs" role="tablist" aria-label={t('settings-hub.title')}>
   <button type="button" role="tab" aria-selected={settingsView === 'general'} className={settingsView === 'general' ? 'active' : ''} onClick={() => selectSettingsView('general')}>{t('settings-hub.general')}</button>
   <button type="button" role="tab" aria-selected={settingsView === 'connection'} className={settingsView === 'connection' ? 'active' : ''} onClick={() => selectSettingsView('connection')}>{t('settings.direct.title')}</button>
   <button type="button" role="tab" aria-selected={settingsView === 'updates'} className={settingsView === 'updates' ? 'active beta5-update-tab' : 'beta5-update-tab'} onClick={() => selectSettingsView('updates')}>{t('settings.updates.eyebrow')}{appUpdate.updateAvailable && <span className="beta5-tab-badge" aria-label={t('appUpdate.available')}>●</span>}</button>
   <button type="button" role="tab" aria-selected={settingsView === 'hardware'} className={settingsView === 'hardware' ? 'active' : ''} onClick={() => selectSettingsView('hardware')}>{t('settings.ledTopology.title')}</button>
  </div>
 </section>
