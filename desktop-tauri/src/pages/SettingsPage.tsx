...
import { Clock3, DownloadCloud, FolderOpen, Globe2, Languages, PlugZap, Save, ShieldCheck, Wifi } from 'lucide-react';
...
interface SettingsPageProps { platform:string; otaSupported:boolean; config:ConnectionConfig; busy:boolean; onChange:(config:ConnectionConfig)=>void; onSave:()=>void; onTest:()=>void; otaPassword:string; onOtaPasswordChange:(value:string)=>void; appUpdate:AppUpdateState; }
...
export function SettingsPage({platform,otaSupported,config,busy,onChange,onSave,onTest,otaPassword,onOtaPasswordChange,appUpdate}:SettingsPageProps){
 const detectedTimezone=Intl.DateTimeFormat().resolvedOptions().timeZone||'UTC';
 const applyTimezone=(timezoneId:string,timezoneAuto=config.timezoneAuto)=>{const snap=timezoneSnapshot(timezoneId);onChange({...config,timezoneId,timezoneAuto,...snap});};
