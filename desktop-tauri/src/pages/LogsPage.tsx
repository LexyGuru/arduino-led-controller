...
export function LogsPage({ arduino, network }: { arduino: ArduinoLog[]; network: NetworkLog[] }) {
  return <div className="page logs-layout"><section className="panel"><p className="eyebrow">ARDUINO</p><h2>Eseménynapló</h2><div className="log-list">{arduino.length ? arduino.map((log,i) => <div key={`${log.timestamp}-${i}`}><time>{log.timestamp}</time><b>{log.type}</b><span>{log.message}</span></div>) : <p className="muted">Nincs elérhető Arduino-esemény.</p>}</div></section>
