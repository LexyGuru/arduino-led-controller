...
type DiagnosticsWarning = { id: string; message: string };
...
function sampleFromDiagnostics(value: ArduinoDiagnosticsResult | null): DiagnosticsTrendSample | null {
  if (!value?.supported || !value.diagnostics) return null;
...
function deltaLabel(previous:number|null,latest:number|null,suffix=''){ if(previous==null||latest==null)return '—'; const delta=latest-previous; return `${delta>0?'+':''}${delta}${suffix}`; }
function buildWarnings(history:DiagnosticsTrendSample[]):DiagnosticsWarning[]{
  const latest=history.at(-1), previous=history.at(-2); if(!latest)return []; const warnings:DiagnosticsWarning[]=[];
  if(latest.rssi!=null&&latest.rssi<=-75) warnings.push({id:'wifi',message:`Weak Wi-Fi: ${latest.rssi} dBm`});
  if(previous&&latest.httpTimeouts!=null&&previous.httpTimeouts!=null&&latest.httpTimeouts>previous.httpTimeouts) warnings.push({id:'http-timeouts',message:'HTTP timeout counter increased'});
  if(previous&&latest.httpRejected!=null&&previous.httpRejected!=null&&latest.httpRejected>previous.httpRejected) warnings.push({id:'http-rejected',message:'HTTP rejected counter increased'});
  if(previous&&latest.httpServerErrors!=null&&previous.httpServerErrors!=null&&latest.httpServerErrors>previous.httpServerErrors) warnings.push({id:'http-server-errors',message:'HTTP server-error counter increased'});
  if(latest.httpMaxMs!=null&&latest.httpMaxMs>=500) warnings.push({id:'http-latency',message:`HTTP max latency ${latest.httpMaxMs} ms`});
  if(latest.schedulerMaxUs!=null&&latest.schedulerMaxUs>=5000) warnings.push({id:'scheduler',message:`Scheduler peak ${latest.schedulerMaxUs} µs`});
  if(latest.renderMaxUs!=null&&latest.renderMaxUs>=20000) warnings.push({id:'render',message:`Render peak ${latest.renderMaxUs} µs`});
  if(latest.statusMaxUs!=null&&latest.statusMaxUs>=10000) warnings.push({id:'status-build',message:`Status build peak ${latest.statusMaxUs} µs`});
  if(previous&&latest.eepromChangedBytes!=null&&previous.eepromChangedBytes!=null&&latest.eepromChangedBytes-previous.eepromChangedBytes>=512) warnings.push({id:'eeprom',message:'EEPROM changed-byte counter jumped by ≥512 B'});
  return warnings;
}
...
export function DiagnosticsPanel({
  ...
  return (
    <section
      className={`panel v791-diagnostics-panel ${
        degraded ? 'recovering' : ''
      }`}
      data-diagnostics-supported={
        supported == null ? 'unknown' : String(supported)
      }
      data-diagnostics-boot-epoch={latestSample?.bootEpoch ?? 'unknown'}
      ...
      <div className="beta2-health-state">
        ...
        <span>
          {error
            ? error
            : supported === false
              ? 'Firmware diagnostics unavailable'
              : 'Read-only runtime telemetry'}
        </span>
      </div>
