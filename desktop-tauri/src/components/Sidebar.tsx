...
export function Sidebar({ page, onChange, appVersion, firmwareVersion, updateAvailable = false, latestAppVersion }: SidebarProps) {
    const { t } = useI18n();
    return (<aside className="sidebar core-sidebar core-v3-sidebar">
      <div className="brand core-brand core-v3-brand">
        ...
        <span className="beta4-version-pill core-v3-release-pill">{appVersion}</span>
      </div>

      <div className="core-nav-label">{t('core.navigation')}</div>
      <nav aria-label={t('core.primaryNavigation')}>
        {items.map(({ id, key, icon: Icon }) => (<button type="button" key={id} className={page === id ? 'active' : ''} data-nav-id={id} data-nav-active={page === id ? 'true' : 'false'} aria-current={page === id ? 'page' : undefined} onClick={() => onChange(id)}>
            <span className="core-nav-icon"><Icon size={19}/></span>
            <span>{t(key)}</span>
            {id === 'settings' && updateAvailable && (<i className="v55-sidebar-update-dot" aria-label={t('appUpdate.available')}/>)}
