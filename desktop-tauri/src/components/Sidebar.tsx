...
export function Sidebar({page,onChange,appVersion,firmwareVersion,otaSupported}:SidebarProps){
 const {t}=useI18n();
 return <aside className="sidebar"><div className="brand"><span className="brand-mark">L</span><div><strong>LED Controller</strong><small>{t('brand.directArduino')}</small></div></div><nav>{items.filter(item=>otaSupported||item.id!=='firmware').map(({id,key,icon:Icon})=><button key={id} className={page===id?'active':''} onClick={()=>onChange(id)}><Icon size={19}/><span>{t(key)}</span></button>)}</nav><div className="sidebar-footer"><small>Arduino UNO R4 WiFi</small><b>{t('sidebar.firmware',{version:firmwareVersion??'–'})}</b><span>{t('sidebar.application',{version:appVersion})}</span></div></aside>;
}
