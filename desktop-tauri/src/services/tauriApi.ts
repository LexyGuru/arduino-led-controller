...
const APP_VERSION='5.7.0-beta.3',CFG='alc.shared.lxc.config.v1',BACKUPS='alc.shared.lxc.schedule-backups.v1';
...
async function fwStatus(updateChannel?:'stable'|'beta',firmwareUpdateChannel?:'stable'|'beta'):Promise<FirmwareStatus>{
  ...
  const available=list[0]??(Array.isArray(cat?.artifacts)&&cat.artifacts[0]?art(cat.artifacts[0]):undefined),installed=String(st?.firmwareVersion??''),ok=rt?.configured===true;
