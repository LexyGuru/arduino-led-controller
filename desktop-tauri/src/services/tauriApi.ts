...
const APP_VERSION='5.6.1-beta.1',CFG='alc.shared.lxc.config.v1',BACKUPS='alc.shared.lxc.schedule-backups.v1',TOKEN='alc.shared.lxc.ota-control-token';
...
async function releases(channel?:'stable'|'beta'){const c=config();const selected=channel??c.firmwareUpdateChannel;const v=await json<any>(`/api/v1/server/firmware/releases?channel=${encodeURIComponent(selected)}`);return(Array.isArray(v?.artifacts)?v.artifacts:[]).map(art)}
...
export const tauriApi={
  ...
  firmwareInstallRelease:(tag:string,channel?:'stable'|'beta'):Promise<FirmwareStatus>=>isTauriRuntime()?invoke('firmware_install_release',{tag,channel}):install(tag),
