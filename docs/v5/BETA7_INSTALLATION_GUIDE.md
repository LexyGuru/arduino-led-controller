# Arduino LED Controller 5.0.0-beta.7 telepítési útmutató

A Beta.7 GitHub prerelease tagje: `v5.0.0-beta.7`. Minden fájl ugyanabból a commitból készül. Telepítés előtt ellenőrizd a `SHA256SUMS` fájlt. A `RELEASE-MANIFEST.json`, `SBOM.cdx.json`, `PROVENANCE.json` és `SECRET-SCAN.json` a kiadás eredetét és integritását dokumentálja.

## Windows x86_64

1. Töltsd le az `Arduino_LED_Controller_5.0.0-beta.7_Windows_x86_64_Setup.exe` fájlt.
2. Ellenőrizd az SHA-256 értéket PowerShellben:

```powershell
Get-FileHash .\Arduino_LED_Controller_5.0.0-beta.7_Windows_x86_64_Setup.exe -Algorithm SHA256
```
...
1. Töltsd le az `Arduino_LED_Controller_5.0.0-beta.7_macOS_Apple_Silicon.dmg` fájlt.
2. Ellenőrzés:

```bash
shasum -a 256 Arduino_LED_Controller_5.0.0-beta.7_macOS_Apple_Silicon.dmg
```
...
Az Intel Mac gépekhez külön `Arduino_LED_Controller_5.0.0-beta.7_macOS_Intel.dmg` készül. A telepítés és az ellenőrzés megegyezik az Apple Silicon lépéseivel.

## Linux x86_64 – AppImage

```bash
sha256sum Arduino_LED_Controller_5.0.0-beta.7_Linux_x86_64.AppImage
chmod +x Arduino_LED_Controller_5.0.0-beta.7_Linux_x86_64.AppImage
./Arduino_LED_Controller_5.0.0-beta.7_Linux_x86_64.AppImage
```

## Debian/Ubuntu x86_64 – DEB

```bash
sha256sum Arduino_LED_Controller_5.0.0-beta.7_Linux_x86_64.deb
sudo apt install ./Arduino_LED_Controller_5.0.0-beta.7_Linux_x86_64.deb
```

## Android

- Aláíró secretek esetén: `Arduino_LED_Controller_5.0.0-beta.7_Android.apk`.
- Aláíró secretek nélkül: `Arduino_LED_Controller_5.0.0-beta.7_Android_debug.apk`.
...
A `Arduino_LED_Controller_5.0.0-beta.7_iOS_iPadOS_unsigned.ipa` nincs Apple tanúsítvánnyal aláírva. Telepítés előtt saját Apple ID/Developer aláírás vagy sideload eszköz szükséges. Ez nem App Store csomag.
...
```bash
curl -fLO https://github.com/LexyGuru/arduino-led-controller/releases/download/v5.0.0-beta.7/install-beta-lxc.sh
...
A Beta.7 schedule-kezeléséhez nem szükséges V5/Node/LXC szerver; az Arduino Direct API az elsődleges adatforrás.

## Beta.7 nyelvi ellenőrzés
...
A Beta.7 Direct API protokollja változatlan, de a jelenlegi ajánlott Beta firmware önálló időzóna- és DST-kezelést tartalmaz. A támogatott párosítás:
