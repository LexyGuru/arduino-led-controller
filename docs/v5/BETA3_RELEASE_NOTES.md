# Arduino LED Controller 5.0.0-beta.3

A Beta.3 a `4.3.0-beta.1` firmware és a Direct API `1.0.0` változatlan, hardveresen elfogadott alapjára épül. A kiadás legfontosabb célja, hogy a Tauri heti időzítése közvetlenül és adatvesztés nélkül szinkronizálja az Arduino teljes, legfeljebb 60 rekordos schedule-listáját.
...
A firmware verziója szándékosan nem változik. A Beta.3 kizárólag a Tauri schedule-kliensét, a felületi állapotkezelést, a regressziós teszteket és a kiadási dokumentációt módosítja.

## Schedule-adatbiztonság

A Beta.3 előtti alkalmazással nem ajánlott hiányos, például 28/60 eltérést mutató schedule-listát visszamenteni. A Beta.3 a Mentés és Törlés műveletet addig nem engedélyezi, amíg az Arduino teljes listája, revisionje és rekordszáma nincs ellenőrizve.
...
A Node.js/LXC csomag kompatibilitási artifactként továbbra is elkészül, de a Beta.3 schedule-funkciójához nem szükséges és nem elsődleges.
...
A release tartalmaz `SHA256SUMS`, `RELEASE-MANIFEST.json`, `latest-beta.json`, `SBOM.cdx.json`, `PROVENANCE.json` és `SECRET-SCAN.json` fájlokat. Minden platformartifact ugyanabból a `v5.0.0-beta.3` release commitból készül.
