...
rm -rf "$STAGE/rust/target" "$STAGE/web-lxc/node_modules" "$STAGE/web-lxc/dist"
...
cp "${ROOT}/deploy/rust-lxc-update.env.example" "$STAGE/deploy/"
...
cp "${ROOT}/deploy/systemd/arduino-led-controller-update.timer" "$STAGE/deploy/systemd/"
cp "${ROOT}/deploy/systemd/arduino-led-controller-update.service" "$STAGE/deploy/systemd/"
