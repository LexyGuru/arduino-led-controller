...
# BEGIN BETA10 CURRENT RELEASE DOC GATE
for beta10_current_doc in \
  docs/v5/BETA10_RELEASE_CHECKLIST.md \
  docs/v5/BETA10_INSTALLATION_GUIDE.md \
  docs/v5/BETA10_RELEASE_NOTES.md
do
  if [ ! -f "${beta10_current_doc}" ]; then
    echo "HIBA: hiányzó Beta.10 current release dokumentum: ${beta10_current_doc}" >&2
    exit 1
  fi
done
# END BETA10 CURRENT RELEASE DOC GATE
