...
assert(current.includes('pkg.version,"5.0.0-beta.8"'), 'current-version package assertion not migrated');
assert(current.includes('BETA8_CURRENT_STATE.md'), 'current state test not migrated to Beta.8 file');
assert(current.includes('`5.0.0-beta.8`'), 'current state app version marker missing');
...
assert(docStatus.includes("'5.0.0-beta.8'"), 'documentation status app version not migrated');
...
assert(readme.includes('| Alkalmazás | `5.0.0-beta.8` |'), 'README current app table not Beta.8');
...
assert(state.includes('`5.0.0-beta.8`'), 'Beta.8 state app version missing');
