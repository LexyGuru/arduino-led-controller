...
assert.ok(workflow.includes('EXPECTED_VERSION: 5.0.0-beta.8'));
...
assert.ok(unit.includes('Description=Arduino LED Controller 5.0.0-beta.8 Staging'));
assert.ok(bundle.includes("candidate: 'beta.8-gate'"));

assert.ok(workflowTest.includes('BETA8_RELEASE_NOTES'));
assert.ok(assetsTest.includes("5.0.0-beta.8"));
...
assert.ok(notes.includes('5.0.0-beta.8'));
