...
assert(openapi.info.version === '5.5.1-beta.1', 'OpenAPI must be Beta.9');
...
try {
  ...
  assert(result.version === '5.5.1-beta.1', 'generator version mismatch');
  ...
  for (const file of expectedFiles) {
    ...
    if (file !== 'index.ts') {
      assert(
        committed.includes('/* OpenAPI verzió: 5.5.1-beta.1 */'),
        `Beta.9 generated header missing: ${file}`
      );
    }
  }

  console.log('OPENAPI_VERSION=5.5.1-beta.1');
