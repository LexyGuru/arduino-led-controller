...
assert.match(api,/function controlToken\(force=false\)/);
assert.match(api,/localStorage\.getItem\(TOKEN\)/);
assert.match(api,/localStorage\.setItem\(TOKEN,v\)/);
assert.match(api,/NEM az Arduino OTA-jelszót/);
...
assert.match(api,/body:JSON\.stringify\(\{version,channel:selected\}\)/);
assert.match(api,/if\(\(e as any\)\?\.status!==401\)throw e;await request\(true\)/);
...
console.log('V695B_LXC_CONTROL_TOKEN_PERSISTED=PASSED');
console.log('V695B_LXC_STALE_CONTROL_TOKEN_RECOVERY=PASSED');
