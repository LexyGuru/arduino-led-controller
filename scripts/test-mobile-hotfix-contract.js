...
assert.match(settings, /OTA firmware-frissítés letiltva/);
assert.match(settings, /Androidon és iOS-en az OTA telepítés, újratelepítés és rollback nem támogatott/);
assert.match(rust, /cfg!\(any\(target_os = "android", target_os = "ios"\)\)[\s\S]*?serde_json::to_vec_pretty\(&config\)/);
assert.match(controller, /Mobilos Arduino-kapcsolati profil és X-Device-Key tartósan mentve/);
