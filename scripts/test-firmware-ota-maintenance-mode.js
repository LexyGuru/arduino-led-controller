...
assert.doesNotMatch(exclusive,/WiFi\.disconnect|ArduinoOTA\.end|EEPROM|server\.begin/);
...
assert.doesNotMatch(started,/showOtaIndicator|logEvent|stopNtpUdpService|EEPROM|handleHttp|runArduinoSchedules/);
const before=body("void otaBeforeApply()","void otaTransferError");
assert.match(before,/showMatrix\(MATRIX_OK\)/);
assert.doesNotMatch(before,/showOtaIndicator|logEvent|EEPROM|WiFi|ntpUdp|server|strip\[/);
const prepare=body("bool prepareOtaService()","float effectScale");
assert.match(prepare,/enterOtaExclusiveMode\(\)/);
assert.doesNotMatch(prepare,/showOtaIndicator|logEvent/);
...
assert.ok(poll>=0&&gate>poll&&http>gate,"OTA exclusive gate sorrend hibás");
console.log("OK: OTA Exclusive Mode csak WiFi + ArduinoOTA + LED Matrix útvonalat hagy aktívan");
console.log("OK: NTP UDP és NeoPixel leáll /ota/prepare után, logolás némítva");
